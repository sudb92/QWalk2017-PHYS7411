'''
Phase-2 implements a Quantum Random walk on a 2d-lattice under the Hadamard operator.
...
#Define the Hadamard coin
HCOIN = np.zeros(shape=(2,2,2,2),dtype=np.complex_)

HCOIN[0,0,0,0] = HCOIN[0,0,0,1] = HCOIN[0,0,1,0] = HCOIN[0,0,1,1] = 1.0/2.
HCOIN[0,1,0,0] = HCOIN[1,0,0,0] = HCOIN[1,1,0,0] = HCOIN[1,1,1,1] = 1.0/2.
HCOIN[0,1,1,0] = HCOIN[1,0,0,1] = 1.0/2.
HCOIN[0,1,0,1] = HCOIN[0,1,1,1] = HCOIN[1,0,1,0] = HCOIN[1,0,1,1] = HCOIN[1,1,0,1] = HCOIN[1,1,1,0] = -1./2.

#Test the definition of HCOIN
#'''
for i in range(2):
    for j in range(2):
        for k in range(2):
...
def toss(state0,j,k,x,y):
    ...
    for jp in np.arange(2):
        for kp in np.arange(2):
            out += HCOIN[jp,kp,j,k]*state0[jp,kp,x,y]
    return out
...
while(timestep<=N):
    ...
    ax.imshow(xy_probability,cmap='coolwarm',animated=True,origin='lower',extent=[-N,N,-N,N])
