'''
Phase-2 implements a Quantum Random walk on a 2d-lattice under the Fourier operator.
...
#Define Fourier coin
FCOIN = 0.5*np.ones(shape=(2,2,2,2),dtype=np.complex_)

FCOIN[1,1,1,1] = FCOIN[0,1,0,1] = (1.0/2.)*1j
FCOIN[0,1,1,0] = FCOIN[1,0,0,1] = FCOIN[1,0,1,1] = FCOIN[1,1,1,0] = -1.0/2.
FCOIN[0,1,1,1] = FCOIN[1,1,0,1] = (-1./2.)*1j

 
#Test the definition of FCOIN
for i in range(2):
    for j in range(2):
        for k in range(2):
            for l in range(2):
...
#Case-4 : Grover-Symmetric initial condition
#state0[0,0,N,N] = 0.5
#state0[0,1,N,N] = -0.5
#state0[1,0,N,N] = -0.5
#state0[1,1,N,N] = 0.5

#Case-5 : Fourier-Symmetric initial conditions
state0[0,0,N,N] = 0.5
state0[0,1,N,N] = 0.5*(1.-1.j)/np.sqrt(2.)
state0[1,0,N,N] = 0.5
state0[1,1,N,N] = -0.5*(1.-1.j)/np.sqrt(2.)
...
def toss(state0,j,k,x,y):
    ...
    for jp in np.arange(2):
        for kp in np.arange(2):
            out += FCOIN[jp,kp,j,k]*state0[jp,kp,x,y]
    return out
